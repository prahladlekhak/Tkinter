...
def login():
   sign_up.destroy()
   import login
b1=Button(text="Login",command=login).grid(row=4,column=2)
...
def register():
   ...
   def values():
      ...
      try:
         ...
         mycursor.execute("insert into tkinter values('{}','{}')".format(a, b))
         con.commit()


   b12=Button(text="Submit",command=values).grid(row=4,column=1)

b2=Button(text="Register",command=register).grid(row=4,column=1)
sign_up.mainloop()